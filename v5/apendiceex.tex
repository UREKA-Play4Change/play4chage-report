...
O nível de audiência (\texttt{AudienceLevel}) é traduzido para uma descrição textual antes de ser injectado no \textit{prompt}:
...
Cada \texttt{ErrorPattern} é traduzido numa instrução específica de diagnóstico antes de ser injectado no \textit{prompt}:
...
Os diagramas apresentados no corpo do relatório (Capítulo~\ref{cap:desenvolvimento}) focam-se em subfluxos isolados, de forma a manter cada figura legível junto do texto que a acompanha. Este apêndice reúne três vistas de conjunto, pensadas para consulta integral: o fluxo de autenticação ponta-a-ponta, o \textit{pipeline} completo de criação de um tópico (ingestão, geração via IA e indexação vectorial), e o diagrama entidade-relação do esquema relacional na íntegra.

\section{Diagrama de Sequência --- Autenticação Completa (\textit{Magic Link})}

A Figura~\ref{fig:ap-auth-full} mostra o fluxo integral de autenticação sem password, desde a submissão do e-mail até à emissão do par de tokens (\textit{access} + \textit{refresh}), incluindo a normalização do e-mail, o \textit{rate limiting} por IP, o hash SHA-256 do token, a operação atómica de consumo do \textit{magic link} (que previne condições de corrida TOCTOU) e a criação automática de conta para utilizadores novos.

\begin{figure}[h!]
    \centering
    \resizebox{\textwidth}{!}{\includegraphics{./figures/seq-demo-auth.png}}
    \caption{Diagrama de sequência completo --- autenticação passwordless por magic link}
    \label{fig:ap-auth-full}
\end{figure}

\section{Diagrama de Sequência --- Pipeline Completo de Ingestão e Geração RAG}

A Figura~\ref{fig:ap-rag-full} mostra o \textit{pipeline} completo accionado pela submissão de um novo tópico: extracção de conteúdo (\textit{Playwright}/\textit{Unstructured}), transição de fases (\texttt{INGESTION} $\to$ \texttt{ANALYSIS} $\to$ \texttt{GENERATION} $\to$ \texttt{INDEXING} $\to$ \texttt{ACTIVE}), segmentação em \textit{chunks}, chamada ao LLM por \textit{chunk}, geração de \textit{embeddings}, deduplicação semântica via pgvector e notificação de progresso em tempo real por \textit{Server-Sent Events}.

\begin{figure}[h!]
    \centering
    \resizebox{\textwidth}{!}{\includegraphics{./figures/seq-demo-pipeline.png}}
    \caption{Diagrama de sequência completo --- pipeline de ingestão, geração e indexação RAG}
    \label{fig:ap-rag-full}
\end{figure}
...
Este apêndice lista todas as variáveis de ambiente configuráveis no sistema, tal como definidas em \texttt{application.yml} (servidor), \texttt{docker-compose.yml} (orquestração de contentores) e nos ficheiros \texttt{.env.example} das aplicações cliente. Os valores reais (segredos, chaves de API, credenciais) não são reproduzidos; apenas os nomes das variáveis e o respectivo propósito.
...
Os restantes parâmetros do modelo de IA (\texttt{ai.mistral.model}, \texttt{ai.mistral.temperature}, \texttt{ai.mistral.timeout-seconds}, tamanho da \textit{pool} de geração, limiares de deduplicação) são definidos directamente em \texttt{application.yml} e não variam por ambiente — apenas a chave de API é externalizada como segredo.

\subsection*{Armazenamento de Objectos (MinIO)}
...
\texttt{SPRING\_PROFILES\_ACTIVE}      & Perfil Spring activo (ex.\ \texttt{prod}, \texttt{dev}) \\
\hline
\end{tabular}
\end{table}
