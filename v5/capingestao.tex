...
Esta validação mitiga ataques de SSRF, impedindo que o sistema seja usado como \textit{proxy} para alcançar serviços internos da rede Docker.
...
\hline
1 & Rate Limiting & \texttt{RateLimitFilter} (Secção~\ref{sec:pipeline}), limites conservadores por IP antes de qualquer autenticação, dado tratar-se da operação de maior custo computacional e financeiro do sistema (limites na Tabela C.2, Apêndice C) & 429 \\
